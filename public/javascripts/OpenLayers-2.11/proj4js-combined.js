  ...
  */
  forward : function(p) {
    ...
    if (this.sphere) {  /* spherical form */
      ...
      ///FIXME: Nie refaktoryzować poniższych czterech linii! Jest błąd w Opera Windows w tych formułach!
      var calcX = (this.k0 * n * al * (1.0 + als / 6.0 * (1.0 - t + c + als / 20.0 * (5.0 - 18.0 * t + Math.pow(t,2) + 72.0 * c - 58.0 * this.ep2))));
      x = calcX + this.x0;
      var calcY = (this.k0 * (ml - this.ml0 + n * tq * (als * (0.5 + als / 24.0 * (5.0 - t + 9.0 * c + 4.0 * Math.pow(c,2) + als / 30.0 * (61.0 - 58.0 * t + Math.pow(t,2) + 600.0 * c - 330.0 * this.ep2))))));
      y = calcY + this.y0;
    }
    p.x = x; p.y = y;
    return p;
